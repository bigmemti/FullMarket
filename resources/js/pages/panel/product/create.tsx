...
import { Category, Brand, BreadcrumbItem } from "@/types";
...
export default function Create({ categories, brands }: { categories: Category[], brands: Brand[] }) {
    ...
    const submit = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        post(route('panel.product.store'), {
            preserveScroll: true,
        });
    };
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <FormContainer className="mb-4">
                <Form onSubmit={submit}>
                    <TextInput label="Name" name="name" value={data.name} onChange={handleNameChange} errors={errors.name} autoFocus />
